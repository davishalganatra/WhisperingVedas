...
def main():
    ...
    try:
        ...
        valid_texts = [t for t in texts if t["text"]]  # Filter out failed parses
        if not texts:
            log_message("WARNING: No input files found in data/inputs", LOG_FILE)
            print("WARNING: No input files found (.txt, .pdf, .wav) in data/inputs")
        elif not valid_texts:
            log_message("ERROR: No valid input files could be parsed", LOG_FILE)
            print("ERROR: All input files failed to parse. Ensure valid .txt, .pdf, or .wav files.")
            sys.exit(1)
        else:
            log_message(f"Found {len(texts)} input files, {len(valid_texts)} valid", LOG_FILE)
    except Exception as e:
    ...
    try:
        template_gen.process_texts(valid_texts)
        log_message(f"Generated templates for {len(valid_texts)} files", LOG_FILE)
    except Exception as e:
    ...
    if not output_files:
        log_message("ERROR: No output files generated", LOG_FILE)
        print("ERROR: No output files were created in data/knowledge_parsed")
        sys.exit(1)
    else:
